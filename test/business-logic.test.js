...
describe('링크 데이터 생성', () => {
  ...
  it('유효하지 않은 URL로 에러를 던진다', () => {
    expect(() => createLinkData('invalid-url')).toThrow('유효하지 않은 URL 형식입니다')
  })
})
...
describe('OG 태그 추출', () => {
  ...
  it('description이 없으면 meta description을 사용한다', () => {
    const html = '<html><head><meta name="description" content="일반 설명"></head></html>'
    const result = extractOGTags(html)
...
describe('Workers AI 응답 파싱', () => {
  const ogData = { title: '제목', description: '설명' }

  it('response 필드가 있으면 사용한다', () => {
    const response = { response: '테스트 응답' }
    expect(parseWorkersAIResponse(response, ogData)).toBe('테스트 응답')
  })

  it('result 필드가 있으면 사용한다', () => {
    const response = { result: '테스트 결과' }
    expect(parseWorkersAIResponse(response, ogData)).toBe('테스트 결과')
  })

  it('응답이 없으면 기본 메시지를 생성한다', () => {
    const result = parseWorkersAIResponse(null, ogData)
...
describe('Discord 임베드 생성', () => {
  it('완전한 임베드를 생성한다', () => {
    ...
    const embed = createDiscordEmbed(linkData)
    
    expect(embed.embeds[0]).toEqual({
      title: '테스트 제목',
      description: '테스트 요약',
      url: 'https://example.com',
      color: 0x0099ff,
  ...
  it('이미지가 없어도 임베드를 생성한다', () => {
    ...
    const embed = createDiscordEmbed(linkData)
    expect(embed.embeds[0].thumbnail).toBeUndefined()
  })
})
