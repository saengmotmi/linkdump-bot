...
export async function setupCloudflareContainer(env: any, ctx?: any) {
  ...
  if (config.storageProvider === "r2") {
    ...
    container.registerInstance(TOKENS.Storage, new R2Storage(env.BUCKET));
  } else {
    ...
    container.registerInstance(
      TOKENS.Storage,
      new FileStorage(config.dataPath || "./data")
    );
  }
  ...
  if (config.aiProvider === "workers-ai") {
    ...
    container.registerInstance(TOKENS.AIClient, new WorkersAIClient(env.AI));
  ...
  if (config.aiProvider === "workers-ai") {
    ...
    container.registerInstance(
      TOKENS.AISummarizer,
      new WorkersAISummarizer(aiClient)
    );
  } else {
  ...
  container.registerInstance(
    TOKENS.Notifier,
    new DiscordNotifier(config.discordWebhooks!)
  );
  ...
  container.registerInstance(
    ...
    new WorkersBackgroundRunner(ctx)
  );
  ...
  container.registerInstance(TOKENS.Runtime, new WorkersRuntime());
